...
const SymptomsPage = () => {
  ...
  useEffect(() => {
    const loadSymptoms = async () => {
      try {
        ...
        const message = axios.isAxiosError(error) ? error.response?.data?.message : 'Не удалось загрузить симптомы';
        toast.error(message || 'Не удалось загрузить симптомы');
      }
    };
    void loadSymptoms();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = axios.isAxiosError(error) ? error.response?.data?.message : 'Не удалось подобрать врачей';
      toast.error(message || 'Не удалось подобрать врачей');
    } finally {
      setIsLoading(false);
    }
  };
                  ...
                  <div className="bg-status-error text-white border-4 border-status-error/30 rounded-[2rem] p-8 flex items-start space-x-6 animate-pulse shadow-xl">
                    ...
                    <div>
                      ...
                      <p className="text-white font-bold text-sm leading-relaxed">
                        Ваше состояние требует приоритетного внимания. Как можно скорее запишитесь к врачу.
                      </p>
                    </div>
                  </div>
              )}
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap justify-center md:justify-start gap-6 mt-4">
                                ...
                                {doctor.specializationName && (
                                    <div className="flex items-center text-sm font-black text-brand-secondary">
                                      <Activity size={18} className="mr-2 text-brand-primary" />
                                      {doctor.specializationName}
                                    </div>
                                )}
                                <div className="flex items-center text-sm font-black text-brand-secondary">
                          ...
                          <button
                              ...
                              onClick={() =>
                                  navigate('/patient/doctors', {
                                    state: { doctorId: doctor.id, symptomIds: selectedSymptoms },
                                  })
                              }
                              className="w-full md:w-auto px-10 py-5 rounded-2xl bg-brand-primary text-white font-black text-lg shadow-xl shadow-brand-primary/20 hover:bg-brand-secondary hover:-translate-y-1 active:scale-95 transition-all duration-300 flex items-center justify-center space-x-3 uppercase tracking-widest text-xs"
                          >
                            <Calendar size={22} />
